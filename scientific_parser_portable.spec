...
def _package_tree(import_name: str, dest: str):
    """PyInstaller onefile: collect_all('flask_caching') may still miss the package; copy tree."""
...
_flask_extra = (
    _package_tree("flask_caching", "flask_caching")
    + _package_tree("flask_limiter", "flask_limiter")
    + _package_tree("limits", "limits")
)

datas = (
    [(os.path.join(SPECDIR, "models"), "models")]
    + [(os.path.join(SPECDIR, "data", "journal_indices.json"), "data")]
    + sklearn_datas
    + pandas_datas
    + certifi_datas
    + webview_datas
...
binaries = (
    sklearn_binaries + pandas_binaries + certifi_binaries + webview_binaries + proxy_binaries
)

_flask_hidden = (
    collect_submodules("flask_caching")
